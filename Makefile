...
corpus-tests-json-schemas.tar.gz: corpus-tests.tar.gz
	...
	# Extract the corpus tests
	@tar -xzf corpus-tests.tar.gz -C /tmp/

	# Convert each .cedarschema file to JSON using Rust Cedar CLI
	@for schema in /tmp/corpus-tests/*.cedarschema; do \
		basename=$$(basename $$schema .cedarschema); \
		echo "Converting $$basename.cedarschema..."; \
		cedar translate-schema --direction cedar-to-json --schema "$$schema" > "/tmp/corpus-tests-json-schemas/$$basename.cedarschema.json" 2>&1 || true; \
	done
