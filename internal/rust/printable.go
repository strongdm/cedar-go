package rust

// Ported from Rust's library/core/src/unicode/printable.rs
// Generated by library/core/src/unicode/printable.py — do not edit by hand.
//
// isPrintable uses the same lookup tables and algorithm as Rust's
// is_printable() function to ensure 1:1 parity with char::escape_debug().

import "unicode"

// isGraphemeExtended returns true if r has the Unicode Grapheme_Extend
// derived property. This matches Rust's char::is_grapheme_extended():
//
//	!self.is_ascii() && unicode::Grapheme_Extend(self)
//
// The Grapheme_Extend property is defined as: Mn + Me + Other_Grapheme_Extend.
func isGraphemeExtended(r rune) bool {
	if r < 0x80 {
		return false
	}
	return unicode.Is(unicode.Mn, r) || unicode.Is(unicode.Me, r) || unicode.Is(unicode.Other_Grapheme_Extend, r)
}

// isPrintable matches Rust's is_printable() from library/core/src/unicode/printable.rs.
func isPrintable(r rune) bool {
	x := uint32(r)
	lower := uint16(x)

	if x < 32 {
		return false
	} else if x < 127 {
		return true
	} else if x < 0x10000 {
		return check(lower, singletons0U, singletons0L, normal0)
	} else if x < 0x20000 {
		return check(lower, singletons1U, singletons1L, normal1)
	} else {
		if 0x2a6e0 <= x && x < 0x2a700 {
			return false
		}
		if 0x2b81e <= x && x < 0x2b820 {
			return false
		}
		if 0x2ceae <= x && x < 0x2ceb0 {
			return false
		}
		if 0x2ebe1 <= x && x < 0x2ebf0 {
			return false
		}
		if 0x2ee5e <= x && x < 0x2f800 {
			return false
		}
		if 0x2fa1e <= x && x < 0x30000 {
			return false
		}
		if 0x3134b <= x && x < 0x31350 {
			return false
		}
		if 0x3347a <= x && x < 0xe0100 {
			return false
		}
		if 0xe01f0 <= x && x < 0x110000 {
			return false
		}
		return true
	}
}

func check(x uint16, singletonsU [][2]byte, singletonsL []byte, normal []byte) bool {
	xupper := byte(x >> 8)
	lowerstart := 0
	for _, ul := range singletonsU {
		upper, lowercount := ul[0], int(ul[1])
		lowerend := lowerstart + lowercount
		if xupper == upper {
			for _, lower := range singletonsL[lowerstart:lowerend] {
				if lower == byte(x) {
					return false
				}
			}
		} else if xupper < upper {
			break
		}
		lowerstart = lowerend
	}

	xi := int32(x)
	current := true
	i := 0
	for i < len(normal) {
		v := normal[i]
		i++
		var length int32
		if v&0x80 != 0 {
			length = int32(v&0x7f)<<8 | int32(normal[i])
			i++
		} else {
			length = int32(v)
		}
		xi -= length
		if xi < 0 {
			break
		}
		current = !current
	}
	return current
}

//nolint:unused // tables referenced by check()
var singletons0U = [][2]byte{
	{0x00, 1},
	{0x03, 5},
	{0x05, 6},
	{0x06, 2},
	{0x07, 6},
	{0x08, 7},
	{0x09, 17},
	{0x0a, 28},
	{0x0b, 25},
	{0x0c, 25},
	{0x0d, 16},
	{0x0e, 12},
	{0x0f, 4},
	{0x10, 3},
	{0x12, 18},
	{0x13, 9},
	{0x16, 1},
	{0x17, 4},
	{0x18, 1},
	{0x19, 3},
	{0x1a, 9},
	{0x1b, 1},
	{0x1c, 2},
	{0x1f, 22},
	{0x20, 3},
	{0x2b, 2},
	{0x2d, 11},
	{0x2e, 1},
	{0x30, 4},
	{0x31, 2},
	{0x32, 1},
	{0xa9, 2},
	{0xaa, 4},
	{0xab, 8},
	{0xfa, 2},
	{0xfb, 5},
	{0xfe, 3},
	{0xff, 9},
}

//nolint:unused // tables referenced by check()
var singletons0L = []byte{
	,
}

//nolint:unused // tables referenced by check()
var singletons1U = [][2]byte{
	{0x00, 6},
	{0x01, 1},
	{0x03, 1},
	{0x04, 2},
	{0x05, 7},
	{0x07, 2},
	{0x08, 8},
	{0x09, 2},
	{0x0a, 5},
	{0x0b, 2},
	{0x0e, 4},
	{0x10, 1},
	{0x11, 2},
	{0x12, 5},
	{0x13, 28},
	{0x14, 1},
	{0x15, 2},
	{0x17, 2},
	{0x19, 13},
	{0x1c, 5},
	{0x1d, 8},
	{0x1f, 1},
	{0x24, 1},
	{0x6a, 4},
	{0x6b, 2},
	{0x6e, 2},
	{0xaf, 3},
	{0xb1, 2},
	{0xbc, 2},
	{0xcf, 2},
	{0xd1, 2},
	{0xd4, 12},
	{0xd5, 9},
	{0xd6, 2},
	{0xd7, 2},
	{0xda, 1},
	{0xe0, 5},
	{0xe1, 2},
	{0xe6, 1},
	{0xe7, 4},
	{0xe8, 2},
	{0xee, 32},
	{0xf0, 4},
	{0xf8, 2},
	{0xfa, 5},
	{0xfb, 1},
}

//nolint:unused // tables referenced by check()
var singletons1L = []byte{
	,
}

//nolint:unused // tables referenced by check()
var normal0 = []byte{
	,
}

//nolint:unused // tables referenced by check()
var normal1 = []byte{
	,
}
